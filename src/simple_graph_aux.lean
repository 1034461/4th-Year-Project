...
def edge_to_finset_powerset_len_two [decidable_pred (∈ G'.verts)] :
Π (e : sym2 V), 
  ...
  begin
    ...
    { rw finset.subset_iff,
      simp only [finset.mem_insert, finset.mem_singleton,
        set.mem_to_finset, forall_eq_or_imp, forall_eq],
      exact ⟨G'.edge_vert h_adj, G'.edge_vert (G'.symm h_adj)⟩, },
    exact ⟨h_subset, ⟨v₁, v₂, v₁_ne_v₂, rfl⟩⟩,
  end ⟩

def edge_to_finset_powerset_len_two_inj [decidable_pred (∈ G'.verts)] : 
∀ e₁ e₂ e₁_in e₂_in,
  G'.edge_to_finset_powerset_len_two e₁ e₁_in = 
    G'.edge_to_finset_powerset_len_two e₂ e₂_in 
  → e₁ = e₂ :=
