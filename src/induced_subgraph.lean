...
lemma induced_subgraph_injective : 
function.injective (induced_subgraph G) :=
begin
...
def set_univ : set G.subgraph := set.image
  (induced_subgraph G) 
  {Rₖ : set V | ∃ fin_Rₖ : fintype ↥Rₖ, 
    (@set.to_finset _ Rₖ (by assumption)).card = k}
...
⟨ (induced_subgraph G) ∘ coe,
  function.injective.comp 
    (induced_subgraph_injective G) finset.coe_injective ⟩
...
lemma coe_eq_complete_graph : 
((complete_graph V).induced_subgraph R).coe = complete_graph ↥R :=
begin
...
def edge_to_finset_powerset_len_two_surj : ∀ s, ∃ e, ∃ e_in,
  ((complete_graph V).induced_subgraph R)
    .edge_to_finset_powerset_len_two e e_in = s :=
begin
  rintro ⟨s, s_in⟩,
  ...
  { rw ← set.mem_to_finset,
    apply finset.mem_of_subset s_subset,
    rw hs,
    simp only [finset.mem_insert, finset.mem_singleton, eq_self_iff_true,
      or_true], },
...
((complete_graph V).induced_subgraph R).edge_finset.card =
  (fintype.card ↥R).choose 2 :=
begin
  suffices : (((complete_graph V).induced_subgraph R).edge_finset).card =
    (finset.powerset_len 2 R.to_finset).card,
  { rw this,
  ...
  refine finset.card_congr 
    (λ e, λ e_in, 
      (((complete_graph V).induced_subgraph R)
        .edge_to_finset_powerset_len_two 
        e (by rwa ← set.mem_to_finset)).val) _ _ _,
  { intros e e_in,
    rw subtype.val_eq_coe,
    have h := (((complete_graph V).induced_subgraph R)
      .edge_to_finset_powerset_len_two e _).property,
    rw subtype.val_eq_coe at h,
    unfold induced_subgraph at h,
    exact h, },
  { simp_rw set.mem_to_finset,
    simp_rw subtype.val_inj,
    exact ((complete_graph V).induced_subgraph R)
      .edge_to_finset_powerset_len_two_inj, },
  { intros s s_in,
